 ...
 *
 * How to build
 *  $ make desktop_dat_to_root
 *
 * How to run
 *  Option 1: 
 *  $ ./desktop_dat_to_root inputFile
 * 
 * Input - 
...
int main(int argc, char **argv){
  ...
  // (header plus samples)
  static const unsigned int header_bytes = 24;
  NS = (HEAD[0] - header_bytes)/sizeof(int);
  ...
  while ( inFile.is_open() && 
    ...
    // HEAD[0] is event size in bytes
    // (header plus samples)
    if( ( HEAD[0] - header_bytes)/sizeof(int) != NS )
      fprintf( stderr, "\n Error: Number of Samples has changed \n ");    
    
    ADC.clear();
    
    //------------------
    // waveform is N lots of 32 bits    
    for (int i = 0; i < (int)NS ; i++){
      inFile.read((char*)&buffer,sizeof(float)); 
      ADC.push_back((short)roundf(buffer));
    }
