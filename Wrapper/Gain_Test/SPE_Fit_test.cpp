...
//		if (r<5)mod = PMT[0];if (r>=5&&r<10)mod = PMT[1];if (r>=10&&r<15) mod = PMT[2];if (r>=15) mod = PMT[3];	[taken out for testing,LK]
		...
		if (mod<10)		
			sprintf(histname, "HV_SPE/PMT_NB000%d_HV%d_test.root",mod, pmtHV[pmtChannel][hv]); 
		if (mod>=10 && mod <100)
			sprintf(histname, "HV_SPE/PMT_NB00%d_HV%d_test.root",mod, pmtHV[pmtChannel][hv]);
		if (mod>=100)
			sprintf(histname, "HV_SPE/PMT_NB0%d_HV%d_test.root",mod, pmtHV[pmtChannel][hv]); 
