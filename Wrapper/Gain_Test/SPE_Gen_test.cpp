...
//		sprintf(hvtest,"hv%d",test);
	...
	for (int w=0; w<4; w++){
		...
		sprintf(filename,"../../BinaryData/PMT0%d_U/GainTest/wave_%d_hv%d.dat",channel[w],w,test);
		printf(filename);
	...
	for (int i=0;i<4;i++){
		...
		if (channel[i]<10)		
			sprintf(histname, "HV_SPE/PMT_NB000%d_HV%d_test.root",channel[i],  Gain[i]);
		if (channel[i]>=10 && channel[i] <100)
			sprintf(histname, "HV_SPE/PMT_NB00%d_HV%d_test.root",channel[i], Gain[i]);
		if (channel[i]>=100)
			sprintf(histname, "HV_SPE/PMT_NB0%d_HV%d_test.root",channel[i],  Gain[i]);
