...
RooAddPdf* makePMTPDF(RooRealVar* counts,double pmval, double psval, double mval, double sval, double f1peval){//, double dmval = 15, double dsval = 20, double fdval = 0.9){
 ...
 RooRealVar* npe = new RooRealVar ("npe","npe", f1peval,0.05,2); // number of photo electrons
...
Result* fitModel(double minval = -30 , double maxval = 1000 ,std::string file = "PMT_NB0171_HV1612_Analysis.root",double max = 10000){
  ...
  //!!!!!!TH1D* histo = (TH1D*)input->Get("SPE1"); 
  TH1D* histo = (TH1D*)input->Get("hCharge1"); 
