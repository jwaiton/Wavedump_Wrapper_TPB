...
{
  return (p[0]/p[1])*sqrt(2.*Pi())*exp(-(pow(x-p[2],2))/(2*pow(p[1],2)));
}

inline double SPE_Fit(double x,double *p)
{
  return (p[3]/p[4])*sqrt(2.*Pi())*exp(-(pow(x-p[5],2))/(2*pow(p[4],2)));
}

inline double second_Photon_Fit(double x,double *p)
{
  //return  p[6]*(p[3]/(sqrt(.2)*p[4]))*sqrt(2.*Pi())*exp(-(pow(x-p[5],2))/(2*pow((sqrt(2.)*p[4]),2)));
  //return p[7] * (  ( (pow (p[4], 2) )/2 ) * exp(-p[6]) * (1/(sqrt(4. * Pi()) * p[4] )) * exp( - (pow(x - 2., 2))/(4. * pow(p[4], 2) ) )   );
  return (p[3]/p[6])*sqrt(2.*Pi())*exp(-(pow(x-p[7],2))/(2*pow(p[6],2)));
...
double fitValParab(double *x,double *p)
{
  //cout << "x0 is " << x[0] << endl;
  return pow(x[0],2)*p[0] + x[0]*p[1] + p[2];
}

double fitSigParab(double *x,double *p)
{
  //cout << "x0 is " << x[0] << endl;
  return pow(x[0],2)*p[0] + x[0]*p[1] + p[2];
  //return (exp(p[0]*(x[0]-p[1])) + p[0]*(x[0]-p[1]) - 1) * p[2];
}
...
//   Gain[4] = atoi(argv[10]);
  ...
  int PMT[5] = {0,0,0,0,0};
	...
	for (int r=0;r<5;r++){
		...
		sprintf(filename,"../outputFile.root");
		...
		//TF1* valleyParab = new TF1("ValleyFit", fitValParab, PedestalPos, SPE_Ped, 3);
		TF1* valleyParab = new TF1("ValleyFit", fitValParab, iMin - 115 , iMin + 220, 3);
		valleyParab->SetParameter(0, 0);
		valleyParab->SetParameter(1, 0);
		valleyParab->SetParameter(2, 0);

		//TF1* PE_Parab    = new TF1("PE_ParabFit", fitSigParab, meanSPE - devSPE, meanSPE + devSPE, 3);
		TF1* PE_Parab    = new TF1("PE_ParabFit", fitSigParab, meanSPE - 0.35*devSPE, meanSPE + 0.35*devSPE, 3);

		PE_Parab->SetParameter(0, 0);
		PE_Parab->SetParameter(1, 0);
		PE_Parab->SetParameter(2, 0);

		// Perform fit.
		//TFitResultPtr tfrp1  =speData->Fit("data1Fit","RSE");
	        TFitResultPtr tfrpVal=speData->Fit("ValleyFit","RSE");
		TFitResultPtr tfrpSig=speData->Fit("PE_ParabFit","RSE");
		...
		addGraph(fitValParab,kGreen+2,valleyParab->GetParameters(), fitXMin, fitXMax);
