...
Result * GetPeakToValley(TString rootFileName = "Run_30_PMT_133_Loc_3_Test_S",string pathToData = "./"){
  ...
  cout << " peak           = " << result->peak.value
       << " (" << result->peak.error         << ") " << endl;

  cout << " valley         = " << result->valley.value
       << " (" << result->valley.error       << ") " << endl;

  cout << " peak to valley = " << result->peakToValley.value
       << " (" << result->peakToValley.error << ") " << endl;

  cout << " peak counts    = " << result->peakCounts.value
       << " (" << result->peakCounts.error   << ") " << endl;

  cout << " valley counts  = " << result->valleyCounts.value
       << " (" << result->valleyCounts.error << ") " << endl;
